...
### 3.15 数学上下标必须整组转换（不能逐字符硬映射）

58. **脚本 body 先递归 LaTeX 转换、再查码点表**（真实事故：用户报告 sigmoid `p = \frac{e^{X\beta}}{1+e^{X\beta}}` 显示成 `p = (eXβ)/(1+eXβ)`——指数完全消失）。旧 `convertScript` 直接对 `^{X\beta}` 的**原始 LaTeX 文本**逐字符映射：`\` 原样、`b`→ᵇ、`e`→ᵉ、`t`→ᵗ、`a`→ᵃ 拼成 `X\ᵇᵉᵗᵃ` 垃圾串，且 `X`/`β` 无 Unicode 上标码点 → 视觉上退化成 `eXβ`。修复：脚本 body 先整体过 `mathToUnicode`（`\beta`→β、`\to`→→ 等命令全部解析掉），再映射；**全部字符有码点才用紧凑 Unicode 上标（`2^{n}`→2ⁿ、`_{i=1}`→ᵢ₌₁），任一字符缺码点则整个回退为显式 `^(Xβ)` / `_(i∈C)` 括号标记**——基线字符混排进脚本（`eXβ`）最不可读，宁可 `e^(Xβ)`。注意两条既有边界不变：无 `{}` 的单字符脚本（`_C`）仍走字面路径（显示 `_C` 而非 `_(C)`）；`\quad` 等未收录命令保留字面。原则：上标回退宁可显式括号，绝不产生视觉歧义。

### 3.16 AskPicker 的高度预算必须以真实可用高度为准（不是全屏预算）

59. **`maxHeight={rows - 2}` 是错误预算**（真实事故：用户报告"选项超过第三行仍会隐藏或溢出至 navigate enter select 这一行"）。rows-2 看似"全屏减两行"，但 AskPicker 实际只占用**内容区**（rows − 输入框 inputAreaHeight − 状态栏 2 行），且内容区里还有 MessageList 先占行。按全屏预算展开选项窗口 → 渲染行数超过容器 → 底部行压到 footer/被裁。**正确预算 = 内容区剩余高度**：ask 模式下消息区让位（`askMessageHeight = clamp(visibleHeight-12, 0, 4)` 只留几行上下文），`askPickerMaxHeight = visibleHeight - askMessageHeight`，MessageList 的 visibleHeight 同步传 `askMessageHeight`（否则它仍按全量渲染挤压 picker）。修复后 AskPicker 的窗口化/截断逻辑（§3.10-40/41）才能真正生效——预算正确，内部算法自会压缩选项。
60. **`QUESTION_CAP=4` 定义了却从未使用**（同事故的一部分）：questionShown 只受 `maxHeight-OVERHEAD-MIN_OPTION_ROWS` 约束，预算大时题干可吃到十几行、挤占选项行。修复：`Math.min(questionRows, questionBudget, QUESTION_CAP)`（题干最多 4 行，选项始终优先）。凡是"常量定义了"就要检查有没有接线，否则注释承诺的行为不存在。
61. **预算必须基于实际内容行数，不能拍脑袋定固定保留值**（第二轮事故：把 ask 模式消息区 `visibleHeight` 固定压到 ≤4 行 → 历史记录几乎全被裁掉，空出区域透出终端黑背景，用户看到"一大块黑幕延伸上去把记录挡住了"；而 AskPicker 内容由选项决定，高度没变，用户觉得"菜单大小没变"）。第三轮事故（同主题）：把选择器最小预算定成 13→16 行，历史多时 `askMessageHeight = visibleHeight - 13` 抢走大量空间 → AskPicker 预算仅 16 行，OVERHEAD 11 扣掉后选项窗口只剩 4 行，长选项 wrap 后选中项吃光预算 → 窗口只有 1 项（用户报 `(showing 1 of 4)`）。**最终正确方案（用户指出的正解：预计算所需高度实时分配）**：把 AskPicker 的整个布局计算提取为共享纯函数 `computeAskPickerLayout(question, options, contentWidth, maxHeight, selectedIndex)`（`AskPicker.tsx` 导出，返回 `{questionShown, optionsBudget, window, totalRows}`），**组件渲染和 App 预算用同一个函数**——App 侧 `askPickerMaxHeight = rows - 2`（注意 ask 模式输入框不渲染，内容区 = rows - 2 而非 visibleHeight！）、`askMessageHeight = askPickerMaxHeight - layout.totalRows`（消息区拿剩余全部），总和恒等于内容区，无黑幕无溢出，任何内容长度都自洽。配套：MessageList 用固定高度容器 `Box height={askMessageHeight} flexShrink={0}` 包裹（其根 Box 是 `flexGrow={1}`，不包会拉伸进空区）、`_repro_askmode.mjs` 复现脚本（用真实 `computeAskPickerLayout` 断言：短选项→选择器矮历史拿多数、长选项→按需取高、总和=内容高度、真实组件渲染不超界）。教训：布局分配永远**先算需求再分配剩余**，需求可计算时就别用固定值/启发式猜；渲染组件和预算分配必须共享同一套布局函数，否则两处漂移。
62. **划词提取的窗口高度必须等于渲染时的窗口高度**（真实事故：选择题菜单弹出后（ask 模式）在历史记录上划词，**高亮位置与复制内容不符**）。根因：ask 模式 MessageList **渲染**时窗口 = `askMessageHeight`（`viewStart = totalLines - askMessageHeight - offset`），但 release 划词时 `extractSelectionText(..., visibleHeight, ...)` 仍传**全量 visibleHeight** → 两边 `viewStart` 不同 → 屏幕行 y → buffer 行映射（`viewStart + (y-1)`）错位。修复：`extractSelectionText` 传 `mode.type === "ask" ? askMessageHeight : visibleHeight`（与渲染同一窗口）。`_smoke_select.mjs` Part D 固化：同一 selection 在窗口=4 时提取 buffer 尾部（'BBB'/'CCC'），在窗口=8 时提取 buffer 头部（'You'）—— 回归护栏断言"全量窗口提取不出渲染窗口内容"。教训：**所有屏幕坐标 → buffer 坐标的换算（渲染高亮、划词提取、滚动）必须使用同一个 visibleHeight**，任何一处用了另一个值就是隐性地图错位；布局改动后必须把"渲染用值"和"提取用值"两个调用点一起检查。
63. **恢复驱动必须单一入口，否则同一会话恢复两次**（真实事故：/continue 打开会话后，"检测到上次未完成的选择题"提示出现**两次**）。根因：`maybeResumeUnfinishedAsk` 有两个调用点——SessionPicker onSelect 显式 `await maybeResumeUnfinishedAsk(opened)` + mount effect（依赖 `[runtime.session]`）。onSelect 里的 `runtime.setSession(opened)` 会改变 `runtime.session` 引用 → effect 再次触发恢复 → 同一会话跑两次（两次提示 + 两次 restoreAsk 注册）。修复：**删掉 onSelect 里的显式调用**，恢复统一由 `[runtime.session]` effect 驱动（setSession 每次赋值新引用必触发；同一引用不重复触发，天然幂等）。教训：异步副作用如果有多个调用点，先检查它们是否会被**同一个状态变更连锁触发**（A 入口改了状态 → B 入口的 effect 依赖该状态 → 双重执行）；把副作用收敛到单一入口，或加幂等守卫（按 session 引用记录已恢复的）。
