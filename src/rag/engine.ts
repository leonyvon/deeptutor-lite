...
import { type DiagnosticResult, diagnoseKB } from "./diagnostics/health.js";
import {
	configureEmbedding,
...
import type { EmbeddingProviderOptions } from "./embedding/provider.js";
...
export interface KnowledgeEngineOptions {
	/** Directory where the RAG index lives (knowledge.db, vectors/). */
	dataDir: string;
	/** Embedding configuration (provider/baseUrl/apiKey/fallback). */
...
export class KnowledgeEngine {
	...
	constructor(options: KnowledgeEngineOptions) {
		this.knowledgeDir = options.dataDir;
		// Standalone app: data dir and embedding config are injected instead of
		// being derived from pi/.omp environment conventions.
		setDefaultKnowledgeDir(options.dataDir);
		configureEmbedding(options.embedding);
	}
	...
	async initialize(knowledgeDir?: string): Promise<void> {
		if (knowledgeDir) this.knowledgeDir = knowledgeDir;
		this.db = openDatabase(this.knowledgeDir);
