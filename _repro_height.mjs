// Repro: rendered input-area row count vs estimateInputLines at narrow widths.
// Mirrors App.tsx input-area structure: Box(paddingX 1, borderTop) > Box(row) > ❯ + TextInput.
...
// Fake stdin so ink's raw-mode/useInput setup works headlessly (like the
// controlled harness in _smoke_parts.mjs). ink reads in readable-stream mode
// and calls setRawMode()/ref()/unref() on the passed stream.
...
async function run(cols) {
  ...
  // count lines that carry content (skip empties)
  ...
  // Block label must not be split mid-word (no isolated "行]" fragments).
